"""
EventsHigh crawler for Hyderabad events
Simple HTTP-based scraper
"""
...
logger = logging.getLogger("crawler.sources.eventshigh")


class EventsHighCrawler(BaseCrawler):
    """Crawler for EventsHigh Hyderabad events"""
    
    BASE_URL = "https://www.eventshigh.com/hyderabad"
    ...
    def __init__(self, data_store=None):
        ...
        super().__init__('eventshigh', config, data_store)
    
    async def fetch_events(self, **kwargs) -> List[Dict[str, Any]]:
        """Fetch events from EventsHigh"""
        events = []
        ...
        async with RateLimitedClient(
            ...
            if not html:
                logger.error("Failed to fetch EventsHigh page")
                return events
            
            soup = BeautifulSoup(html, 'lxml')
            
            # Look for event cards
            event_cards = soup.find_all('div', class_=re.compile('event-card|event-item|listing'))
            
            # Also try generic article/list items
            if not event_cards:
                event_cards = soup.find_all(['article', 'div'], class_=re.compile('event|listing|item'))
            ...
            # If still no events, try meta tags and JSON-LD
            if not events:
    ...
    def _extract_event_from_card(self, card: BeautifulSoup) -> Optional[Dict[str, Any]]:
        ...
        try:
            # Title
            title_elem = card.find(['h2', 'h3', 'h4', 'h1']) or card.find(class_=re.compile('title|name|heading'))
            ...
            # Date
            date_elem = card.find(class_=re.compile('date|time|when'))
            date_text = date_elem.get_text(strip=True) if date_elem else None
            
            # Venue
            venue_elem = card.find(class_=re.compile('venue|location|place|where'))
            ...
            if link_elem:
                ...
                if href.startswith('http'):
                    ...
                    event_url = f"https://www.eventshigh.com{href}"
                else:
                    event_url = f"https://www.eventshigh.com/{href}"
            
            # Category
            category_elem = card.find(class_=re.compile('category|tag|type'))
    ...
    def _extract_from_metadata(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        """Extract events from JSON-LD or meta tags"""
        events = []
