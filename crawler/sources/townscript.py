"""
Townscript crawler for Hyderabad events
"""
...
logger = logging.getLogger("crawler.sources.townscript")


class TownscriptCrawler(BaseCrawler):
    """Crawler for Townscript Hyderabad events"""
    
    BASE_URL = "https://www.townscript.com/in/hyderabad"
    ...
    def __init__(self, data_store=None):
        ...
        super().__init__('townscript', config, data_store)
    ...
    async def fetch_events(self, **kwargs) -> List[Dict[str, Any]]:
        ...
        async with RateLimitedClient(
            ...
            if not html:
                logger.error("Failed to fetch Townscript page")
                return events
            
            soup = BeautifulSoup(html, 'lxml')
            
            # Townscript uses card-based layout
            event_cards = soup.find_all('div', class_=re.compile('event-card|event-listing|card'))
            
            # Fallback
            if not event_cards:
                event_cards = soup.find_all('article', class_=re.compile('event'))
            
            logger.info(f"Found {len(event_cards)} Townscript event cards")
    ...
    def _extract_event_from_card(self, card: BeautifulSoup) -> Optional[Dict[str, Any]]:
        ...
        try:
            title_elem = card.find(['h2', 'h3', 'h4', 'h1']) or card.find(class_=re.compile('title|event-title'))
            title = title_elem.get_text(strip=True) if title_elem else None
            
            if not title or len(title) < 3:
                return None
            
            date_elem = card.find(class_=re.compile('date|event-date|datetime'))
            ...
            price_elem = card.find(class_=re.compile('price|cost|ticket|amount'))
            ...
            if link_elem:
                ...
                if href.startswith('http'):
                    ...
                    event_url = f"https://www.townscript.com{href}"
