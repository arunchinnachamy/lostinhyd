"""
AllEvents crawler for Hyderabad events
"""
...
logger = logging.getLogger("crawler.sources.allevents")


class AllEventsCrawler(BaseCrawler):
    """Crawler for AllEvents.in Hyderabad events"""
    
    BASE_URL = "https://allevents.in/hyderabad/all"
    ...
    def __init__(self, data_store=None):
        ...
        super().__init__('allevents', config, data_store)
    
    async def fetch_events(self, **kwargs) -> List[Dict[str, Any]]:
        """Fetch events from AllEvents"""
        events = []
        ...
        async with RateLimitedClient(
            ...
            if not html:
                logger.error("Failed to fetch AllEvents page")
                return events
            
            soup = BeautifulSoup(html, 'lxml')
            
            # AllEvents uses specific class names
            event_cards = soup.find_all('div', class_=re.compile('event-item|event-card|event-box'))
    ...
    def _extract_event_from_card(self, card: BeautifulSoup) -> Optional[Dict[str, Any]]:
        ...
        try:
            title_elem = card.find(['h2', 'h3', 'h4']) or card.find(class_=re.compile('title|event-title|name'))
            ...
            price_elem = card.find(class_=re.compile('price|ticket|cost'))
            ...
            category_elem = card.find(class_=re.compile('category|tag|event-type'))
